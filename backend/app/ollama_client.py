...
from .security_mapping import THREAT_DEFINITIONS, map_security_implications
...
def generate_threat_analysis(
    ...
    try:
        ...
        llm_failed = True
        fallback_vector = next((vector for vector in attack_vectors if vector["id"] == "social_engineering"), None)
        if fallback_vector is None:
            fallback_vector = THREAT_DEFINITIONS["social_engineering"]
        attack_vectors = [fallback_vector]
        parsed = {
            "final_verdict": label.lower() if label else "uncertain",
            "risk_level": "high" if (confidence or 0) > 0.75 else "medium",
            "score_summary": "Ollama reasoning unavailable; flagging potential social engineering.",
            "artefact_explanation": [
                "Potential social engineering activity detected. Escalate for manual review."
            ],
            "overall_explanation": "LLM reasoning failed; treating as a social-engineering risk only.",
        }
    ...
    if not llm_failed:
        ...
        parsed["overall_explanation"] = "Ollama reasoning unavailable."
        parsed.setdefault(
            "artefact_explanation",
            ["Potential social engineering activity detected. Escalate for manual review."],
        )
    ...
    parsed.update(
        {
            ...
            "ollama_available": not llm_failed,
        }
    )
    return parsed
